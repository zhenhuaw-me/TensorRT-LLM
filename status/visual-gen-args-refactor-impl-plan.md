# VisualGenArgs Refactor — Implementation Plan

> Source design: [`designs/visual-gen-args-refactor.md`](../designs/visual-gen-args-refactor.md) — converged 2026-05-13 (iteration 19) after 9 Codex adversarial-review iterations + multiple owner review passes + PR #9 review rounds 1–4. The plan below is the coding-execution view; the design is the normative spec for every field, name, and signature.

## Goal Description

Refactor the public `VisualGenArgs` engine-config API surface so that adding a new model or a new optimization does not force edits to the shared config class. End-state shape is fully specified in the source design's **§13 Final Design** — sub-configs in `tensorrt_llm.visual_gen.*`, entry-points at top-level `tensorrt_llm.*`, `_config`-suffixed cross-cutting Pydantic sub-configs (mirroring `LlmArgs`), per-architecture knobs in a strict-on-unknown-keys `pipeline_config: dict[str, Any]` resolved through the existing `PIPELINE_REGISTRY` keyed by Diffusers `_class_name`, two discovery classmethods on `VisualGen`, dead code deleted, derived flags hidden from the public schema, and one CLI alias added.

The refactor is a clean break — no `validation_alias`, no `Field(deprecated=...)`, no soft-removal window. VisualGen is pre-GA; users update their YAMLs at this checkpoint.

Each acceptance criterion below corresponds to one migration phase from the source design's **§14.1 Sequencing** and is sized to land in one RLCR round (one commit, one focused PR-style diff against `origin/main`).

## Acceptance Criteria

Following TDD philosophy, each criterion includes positive and negative tests for deterministic verification. All file paths reference the current `origin/main` layout. Test invocations run via the `run_in_gpu_env` helper bound at Stage 2 entry (see Implementation Notes).

- **AC-1: Move `VisualGenArgs` and sub-configs to `tensorrt_llm/visual_gen/args.py`; adopt `Field(status="prototype")` markers.** (§14.1 Phase 1; §13.1; §10.1; §10.3)
  - Positive Tests (expected to PASS):
    - `from tensorrt_llm.visual_gen import VisualGenArgs, CompilationConfig, CudaGraphConfig, TorchCompileConfig, AttentionConfig, ParallelConfig, CacheConfig, TeaCacheConfig, CacheDiTConfig, QuantConfig` imports without error.
    - `from tensorrt_llm import VisualGen, VisualGenArgs, VisualGenParams, VisualGenResult, VisualGenOutput, VisualGenMetrics, ExtraParamSchema` (the seven top-level re-exports on `origin/main`) imports without error.
    - `VisualGenArgs.model_fields[<every field>]` carries `json_schema_extra["status"] == "prototype"` (or higher, when explicitly upgraded).
    - Existing `tests/unittest/_torch/visual_gen/test_visual_gen_args.py` Pydantic round-trip tests pass against the new module path.
  - Negative Tests (expected to FAIL / RAISE):
    - `from tensorrt_llm.visual_gen import VisualGenError` raises `ImportError` (the `VisualGen*Error` names were dropped from the public API on `origin/main`; verified in §13.1).
    - `from tensorrt_llm.visual_gen import PipelineComponent` raises `ImportError` (deliberately *not* exposed publicly — §13.1).
    - `from tensorrt_llm.visual_gen import register_pipeline` and `from tensorrt_llm.visual_gen import _PipelineEntry` both raise `ImportError` (internal — §13.1, §13.8).

- **AC-2: Rename every cross-cutting sub-config attribute on `VisualGenArgs` to the `_config` suffix.** (§14.1 Phase 2; §7.5; §13.3)
  - Positive Tests:
    - `VisualGenArgs.model_fields` contains exactly these sub-config field names: `parallel_config`, `compilation_config`, `cuda_graph_config`, `torch_compile_config`, `attention_config`, `cache_config`, `quant_config`.
    - YAML `parallel_config: { ulysses_size: 2 }` deserializes into `args.parallel_config.ulysses_size == 2` via `VisualGenArgs(**yaml.safe_load(...))`.
    - `from_yaml` / `to_yaml` classmethods on `VisualGenArgs` round-trip a config using only the suffixed names (§12.3).
  - Negative Tests:
    - `VisualGenArgs.model_fields` does NOT contain any of: `compilation`, `parallel`, `attention`, `cache`, `cuda_graph`, `torch_compile`.
    - YAML using the unsuffixed names (e.g. `parallel: { ulysses_size: 2 }`) raises `pydantic.ValidationError` with an unknown-key error (no `validation_alias` shim per §10.2).
    - `args.parallel` (without `_config`) raises `AttributeError`.

- **AC-3: Rename master-switch fields and add `CompilationConfig.skip_warmup`.** (§14.1 Phase 3; §13.4)
  - Positive Tests:
    - `CudaGraphConfig().enable == False` (default) and `CudaGraphConfig(enable=True).enable is True`.
    - `TorchCompileConfig().enable == True` (default; matches §13.4) and the other two `enable_*` sub-options `enable_fullgraph` (default `False`) / `enable_autotune` (default `True`) keep their names and defaults.
    - `CompilationConfig().skip_warmup == False` and setting it to `True` actually skips the warmup pass in `PipelineLoader.load(...)` (verified by an existing or new unit test that asserts the warmup phase short-circuits).
    - `args.cuda_graph_config.enable` and `args.torch_compile_config.enable` are the access paths used by `PipelineLoader` / consumers.
  - Negative Tests:
    - `CudaGraphConfig(enable_cuda_graph=True)` raises `pydantic.ValidationError` (extra="forbid"; field has been renamed).
    - `TorchCompileConfig(enable_torch_compile=True)` raises `pydantic.ValidationError`.
    - Top-level `VisualGenArgs(skip_warmup=True)` raises `pydantic.ValidationError` (the field has moved into `compilation_config`).

- **AC-4: Delete dead-code fields verified unwired at `e527a9f785` (and move `parallel.dit_dim_order` to an internal constant).** (§14.1 Phase 4; §7.6; §7.7; §13.5)
  - Positive Tests:
    - `PipelineConfig` class no longer exists (the `tensorrt_llm/_torch/visual_gen/config.py` module export, or its successor in `tensorrt_llm/visual_gen/args.py`, contains no `PipelineConfig` symbol).
    - `ParallelConfig.model_fields` contains exactly the five surviving fields after AC-9 renames complete: `enable_parallel_vae`, `parallel_vae_split_dim`, `cfg_size` (or temporarily still `dit_cfg_size` until AC-9), `ulysses_size`/`dit_ulysses_size`, `attn2d_size`/`dit_attn2d_row_size`+`dit_attn2d_col_size`. (Final naming is settled by AC-9; AC-4 only deletes the dead ones.)
    - `Mapping(...)` still constructs successfully without `dim_order=` plumbed through `ParallelConfig` — uses `DEFAULT_DIM_ORDER` from `mapping.py` directly.
    - Existing Wan / Flux / LTX-2 smoke tests (e.g. `tests/unittest/_torch/visual_gen/test_*_pipeline.py`) still pass.
  - Negative Tests:
    - Setting any of these via `VisualGenArgs(...)` raises `pydantic.ValidationError`: `dtype`, `device`, `pipeline=...`, `pipeline_config_legacy_name`, `parallel_config.dit_dp_size`, `parallel_config.dit_fsdp_size`, `parallel_config.dit_ring_size`, `parallel_config.dit_tp_size`, `parallel_config.refiner_dit_*` (any of the 7), `parallel_config.t5_fsdp_size`, `parallel_config.dit_dim_order`.
    - `tests/unittest/_torch/visual_gen/test_fused_qkv.py` is either removed or rewritten to no longer reference the deleted `fuse_qkv` field (the test exists today only to round-trip a now-deleted Pydantic field; per §7.6 it has no production reader).
    - Any test that imported `OffloadConfig`-shaped fields (`pipeline.enable_offloading`, `pipeline.offload_device`, `pipeline.offload_param_pin_memory`) is removed (verified zero consumers in tree at `e527a9f785`).

- **AC-5: Move `dynamic_weight_quant` and `force_dynamic_quantization` off the public `VisualGenArgs` schema.** (§14.1 Phase 5; §7.4; §13.3)
  - Positive Tests:
    - `VisualGenArgs.model_fields` does NOT contain `dynamic_weight_quant` or `force_dynamic_quantization`.
    - `VisualGenArgs(...).model_dump()` does NOT contain `dynamic_weight_quant` or `force_dynamic_quantization`.
    - The internal `DiffusionModelConfig` (the merged config built by `PipelineLoader`) still receives both flags correctly, populated by the existing `_parse_quant_config_dict` logic — verified by a test that constructs a `QuantConfig` with dynamic quant settings and inspects the resulting `DiffusionModelConfig`.
    - Existing quantization end-to-end tests (e.g. NVFP4 / FP8 quant smoke tests in `tests/unittest/_torch/visual_gen/`) still pass.
  - Negative Tests:
    - `VisualGenArgs(dynamic_weight_quant=True)` raises `pydantic.ValidationError` (the field is no longer a user-set Pydantic field).
    - `VisualGenArgs(force_dynamic_quantization=True)` raises `pydantic.ValidationError`.
    - The two names do NOT appear in `VisualGenArgs.model_json_schema()` output.

- **AC-6: Relocate test/debug knobs.** (§14.1 Phase 6; §11.1; §13.3, §13.4)
  - AC-6.1: `skip_warmup` lives at `compilation_config.skip_warmup` (already added in AC-3); the flat top-level `VisualGenArgs.skip_warmup` is removed.
    - Positive: `VisualGenArgs(compilation_config=CompilationConfig(skip_warmup=True))` short-circuits the warmup pass.
    - Negative: `VisualGenArgs(skip_warmup=True)` raises `pydantic.ValidationError`.
  - AC-6.2: `skip_components` becomes an env var `TLLM_VG_SKIP_COMP` (comma-separated component names parsed inside `PipelineLoader`); the flat top-level `VisualGenArgs.skip_components` field is removed.
    - Positive: With `monkeypatch.setenv("TLLM_VG_SKIP_COMP", "text_encoder,vae")`, `PipelineLoader` actually skips loading those components. Existing tests that used `VisualGenArgs(skip_components=[...])` are migrated to the env-var fixture (e.g., `tests/unittest/_torch/visual_gen/test_flux_pipeline.py`, `test_cache_dit.py`). A `skip_components_env` fixture is provided in `tests/unittest/_torch/visual_gen/conftest.py` (§11.1).
    - Negative: `VisualGenArgs(skip_components=[...])` raises `pydantic.ValidationError`. `PipelineComponent` is NOT in the public `tensorrt_llm.visual_gen.*` namespace.
  - AC-6.3: `enable_layerwise_nvtx_marker` moves from `pipeline.enable_layerwise_nvtx_marker` to a top-level flat field on `VisualGenArgs`, default `False`, `status="prototype"` — mirrors `TorchLlmArgs.enable_layerwise_nvtx_marker` exactly.
    - Positive: `VisualGenArgs(enable_layerwise_nvtx_marker=True)` causes `PipelineLoader` to register the per-layer NVTX hooks (existing `LayerwiseNvtxMarker` consumer at `pipeline_loader.py:253-260`).
    - Negative: `VisualGenArgs(pipeline=PipelineConfig(enable_layerwise_nvtx_marker=True))` raises `pydantic.ValidationError` (the `pipeline` sub-config is gone).

- **AC-7: Upgrade the existing pipeline registry to carry per-family metadata.** (§14.1 Phase 7; §13.8)
  - Positive Tests:
    - `tensorrt_llm/_torch/visual_gen/pipeline_registry.py` defines a private `_PipelineEntry` dataclass with the four fields: `pipeline_cls: type`, `hf_ids: list[str]`, `defaults: dict[str, Any]`, `doc: str`.
    - `PIPELINE_REGISTRY` is typed `dict[str, _PipelineEntry]`.
    - `@register_pipeline("Foo")` (the existing one-positional-arg form) still works — registers a `_PipelineEntry` with empty `hf_ids`, empty `defaults`, empty `doc`. **Existing pipeline registrations in tree must not need edits at this phase.**
    - `@register_pipeline("Foo", hf_ids=["org/foo"], defaults={"k": "v"}, doc="Foo family.")` registers an entry populated with the kwargs.
    - `AutoPipeline.from_config(...)` reads `PIPELINE_REGISTRY[class_name].pipeline_cls` (one extra attribute access vs. today) and dispatches correctly. All existing pipeline-loading tests continue to pass.
  - Negative Tests:
    - Registering the same `name` twice via `@register_pipeline` raises `ValueError("Pipeline already registered: ...")`.
    - `_PipelineEntry` is not in the public `tensorrt_llm.visual_gen.*` exports; importing it from `tensorrt_llm.visual_gen` raises `ImportError`.

- **AC-8: Add `pipeline_config: dict[str, Any]` to `VisualGenArgs` with strict unknown-key validation at load time.** (§14.1 Phase 8; §9.3; §13.3; §13.9)
  - Positive Tests:
    - `VisualGenArgs.model_fields["pipeline_config"]` exists, default factory is `dict`, `json_schema_extra["status"] == "prototype"`.
    - `args = VisualGenArgs(model="<known HF id>", pipeline_config={"<valid key from defaults>": "<value>"})` constructs and loads successfully.
    - `PipelineLoader` performs `unknown = set(args.pipeline_config) - set(entry.defaults)` and raises `ValueError(f"Unknown pipeline_config keys for {class_name} ({args.model}): {sorted(unknown)}. Valid keys: {sorted(entry.defaults)}")` when given a key not in the matched entry's `defaults`. The error message includes both the offending keys and the valid key set (exact format per §9.3).
    - Merged dict order is `{**entry.defaults, **args.pipeline_config}` — user-supplied values win over registry defaults.
  - Negative Tests:
    - `VisualGenArgs(model="Lightricks/LTX-Video", pipeline_config={"text_encder_path": ""})` (intentional typo) loads in Pydantic but `PipelineLoader.load(...)` raises `ValueError` with the typo key listed under "Unknown pipeline_config keys".
    - Pre-AC-9: with no `defaults` populated on any entry yet, `pipeline_config={"any_key": ...}` raises `ValueError` for every key — confirms strict validation is wired before metadata is filled in.

- **AC-9: Fill per-family decorator metadata; move LTX-2 paths off `VisualGenArgs`; rename surviving `ParallelConfig` fields.** (§14.1 Phase 9; §7.2; §7.7; §13.5; §13.8)
  - AC-9.1: LTX-2 paths move to the registry.
    - Positive: `PIPELINE_REGISTRY["LTX2Pipeline"].defaults == {"text_encoder_path": "", "spatial_upsampler_path": "", "distilled_lora_path": ""}` (set via `@register_pipeline("LTX2Pipeline", defaults={...}, ...)` at the LTX-2 pipeline class site). `VisualGen.pipeline_config("Lightricks/LTX-Video")` returns this dict by value.
    - Negative: `VisualGenArgs.model_fields` does NOT contain `text_encoder_path`, `spatial_upsampler_path`, or `distilled_lora_path`. Setting them as top-level kwargs raises `pydantic.ValidationError`.
  - AC-9.2: `@register_pipeline` calls in `pipeline_wan.py`, `pipeline_flux.py`, `pipeline_ltx2.py` (and any other in-tree pipeline file) populate `hf_ids=[...]` and `doc="..."` per the canonical-id lists in §13.8.
    - Positive: `set(VisualGen.supported_models())` is a superset of `{"Lightricks/LTX-Video", "Wan-AI/Wan2.1-T2V-14B", "black-forest-labs/FLUX.1-dev", ...}` (exact contents per §13.8 examples; deltas allowed only for in-tree pipeline files this design didn't enumerate).
    - Negative: A pipeline class with no `@register_pipeline` decorator does NOT appear in `VisualGen.supported_models()`.
  - AC-9.3: `ParallelConfig` field renames per §13.5.
    - Positive: `ParallelConfig.model_fields` contains exactly: `enable_parallel_vae`, `parallel_vae_split_dim`, `cfg_size`, `ulysses_size`, `attn2d_size`. `attn2d_size` has type `tuple[int, int]` with default `(1, 1)`. `pipeline_loader.py` unpacks `attn2d_size` into the two-int `attn2d_row_size=` / `attn2d_col_size=` constructor args of `Mapping(...)`.
    - Negative: `ParallelConfig(dit_cfg_size=2)` raises `pydantic.ValidationError`. `ParallelConfig(dit_ulysses_size=2)` raises. `ParallelConfig(dit_attn2d_row_size=2, dit_attn2d_col_size=2)` raises.

- **AC-10: Add the discovery API on `VisualGen`.** (§14.1 Phase 10; §9.2; §13.9)
  - Positive Tests:
    - `VisualGen.supported_models()` returns `list[str]` — the union of `entry.hf_ids` across all registered entries (order: per-entry insertion, by registration). Result is a copy (mutating it does not affect `PIPELINE_REGISTRY`).
    - `VisualGen.pipeline_config("Lightricks/LTX-Video")` returns `{"text_encoder_path": "", "spatial_upsampler_path": "", "distilled_lora_path": ""}` (after AC-9.1 lands).
    - `VisualGen.pipeline_config("LTX2Pipeline")` (passing the `_class_name` directly) returns the same dict.
    - `VisualGen.pipeline_config("/path/to/local/ltx-video-checkpoint")` resolves via `AutoPipeline._detect_from_checkpoint(...)` and returns the LTX-2 defaults.
    - The returned dict is a copy — `VisualGen.pipeline_config(m).update({...})` does NOT mutate `PIPELINE_REGISTRY[class_name].defaults`.
  - Negative Tests:
    - `VisualGen.pipeline_config("not-a-real-hf-id/not-real")` raises `KeyError` (no entry matches HF id, name, or local path).
    - `VisualGen.supported_models()` does NOT contain entries that registered without `hf_ids` (e.g., the empty-default case from AC-7).

- **AC-11: CLI updates.** (§14.1 Phase 11; §12.4)
  - Positive Tests:
    - `trtllm-serve --visual_gen_args path/to/config.yaml ...` accepts the new flag and binds to the `visual_gen_args` destination, loading the YAML through `VisualGenArgs.from_yaml(...)` (or the equivalent inline `yaml.safe_load` + `VisualGenArgs(**...)`).
    - `trtllm-serve --extra_visual_gen_options path/to/config.yaml ...` still works (alias bound to the same destination via the click `--visual_gen_args / --extra_visual_gen_options` two-name pattern in §12.4).
    - The same two flags work for `trtllm-bench` (`tensorrt_llm/bench/benchmark/visual_gen.py`).
  - Negative Tests:
    - Passing **both** `--visual_gen_args` and `--extra_visual_gen_options` on the same command line behaves per click's two-name semantics (the latter wins, or click errors — capture and document the actual behavior; whichever it is, document explicitly in the help text).
    - `--config` (the LLM-side flag) does NOT bind to `visual_gen_args`. Passing `--config foo.yaml` to `trtllm-serve` continues to populate `extra_llm_api_options` only; VisualGen YAML must be loaded via `--visual_gen_args`.

## Path Boundaries

This is a fully-determined refactor. The source design has zero open architectural choices that affect this implementation; every public class, field name, default, and import path is fixed by §13. The path boundaries below distinguish the **loop-finalize endpoint** (when Codex emits `COMPLETE`) from the **per-round endpoint** (what one RLCR round delivers).

### Upper Bound (loop-finalize endpoint)

When Codex emits `COMPLETE` and the code-review phase passes, the working branch carries the full §13 Final Design end-state:

- `VisualGenArgs` lives in `tensorrt_llm/visual_gen/args.py`; sub-configs exported from `tensorrt_llm.visual_gen.*`; top-level entry-points (`VisualGen`, `VisualGenArgs`, `VisualGenParams`, `VisualGenResult`, `VisualGenOutput`, `VisualGenMetrics`, `ExtraParamSchema`) re-exported from `tensorrt_llm.*`.
- All eleven ACs satisfied; every test described in each AC has its positive cases passing and its negative cases raising the documented error.
- All in-tree pipeline classes (`WanPipeline`, `WanImageToVideoPipeline`, `FluxPipeline`, `Flux2Pipeline`, `LTX2Pipeline`, plus any others present at `origin/main` checkout time) carry the full `@register_pipeline(name, hf_ids=[...], defaults={...}, doc="...")` metadata.
- All call sites updated (YAML configs in `examples/`, perf-sanity configs, integration tests) — no remaining references to renamed/deleted field names.
- `tests/unittest/_torch/visual_gen/` is green on the target GPU; integration smoke tests for Wan / Flux / LTX-2 still pass.
- No half-states in tracked files: no `validation_alias`, no `Field(deprecated=...)`, no commented-out compat shims, no half-renamed fields.

### Lower Bound (per-round endpoint)

Each RLCR round advances the goal-tracker by **one AC** (typically one phase from §14.1, sometimes two consecutive phases that share consumers when they batch naturally — e.g. AC-2 + AC-3 if the sub-config renames and master-switch renames touch the same files in one diff). The per-round summary moves at least one `pending` Active Task to `completed`, with a commit landing the corresponding tracked-file changes and the round summary pointing to actual test logs captured via `run_in_gpu_env`.

A round is "done" when:
- The targeted AC's positive tests pass on the build machine (logs captured under `.humanize/rlcr/<ts>/logs/`).
- The targeted AC's negative tests behave as specified (capture the assertion).
- All other ACs already marked `completed` continue to pass (no regressions).
- Untargeted ACs may remain `pending` — that's expected mid-loop.

### Allowed Choices

The choice surface is fixed by the source design. The plan does **not** re-open any of:

- **No alternative shape choices.** Top-level shape (Option C: composed cross-cutting sub-configs + dict pipeline_config), `_config` suffix naming, dict-not-typed-Pydantic for per-architecture knobs, `_class_name`-keyed registry, strict unknown-key validation, `Field(status="prototype")` markers — all settled in §5–§13.
- **No backwards-compat shims.** No `validation_alias`, no `AliasChoices`, no `Field(deprecated=...)`, no soft-removal cycle, no `DeprecationWarning` (§10.2, §14.2). Removed and renamed names disappear in the same commit they're touched in.
- **No new public symbols beyond §13.1.** `_PipelineEntry`, `@register_pipeline`, `PIPELINE_REGISTRY`, `PipelineComponent` stay in `tensorrt_llm._torch.visual_gen.*` (internal); promotion to public is explicitly deferred (§13.1, §15.8).
- **No re-introduction of deleted fields under new names.** The deleted fields (§7.6, §7.7) stay deleted — they reappear only when the corresponding feature actually ships (e.g. offload via PR #14095's typed `OffloadConfig`).
- **No CLI dotted-override implementation.** vLLM-style `--cuda_graph_config.enable=true` is out of scope (§12.4, §15 Q7).
- **No API-stability snapshot harness.** The five-file snapshot test harness is deferred to a separate task (§10.2).

The implementation is free to:
- Choose the most natural file layout under `tensorrt_llm/visual_gen/` (single `args.py` vs. one-file-per-sub-config) so long as the imports in §13.1 work.
- Choose whether to land Phase 4's 17 field deletions as one commit or split (the design says either is fine — §14.1 Phase 4 "These removals are independent; can be done together or split"). Splitting maps cleanly to per-round work; bundling is acceptable when commits share test/fixture updates.
- Choose how to migrate existing YAML configs in `examples/` and `tests/` — direct edit per phase, or one batched cleanup pass at the end. Either way, no YAML file is left referencing a renamed or deleted name when the loop finalizes.

## Feasibility Hints and Suggestions

> Reference only. The source design is normative; this section just helps the loop pick natural batching and find the right files.

### Conceptual Approach

Phases 1–6 are mechanical Python edits to `VisualGenArgs` and the existing sub-config classes — module move, attribute renames, master-switch renames, dead-field deletions, derived-flag relocations, and the three test/debug-knob movements. The compile-and-test loop validates that the public schema matches §13 and that consumers (`PipelineLoader`, `Mapping`, examples) keep working.

Phases 7–10 introduce the registry-metadata + discovery affordance. The existing `pipeline_registry.py` is upgraded in place (Phase 7), `VisualGenArgs.pipeline_config` is added (Phase 8) with strict-validation hooked into `PipelineLoader`, decorator sites are populated (Phase 9), and the two classmethods on `VisualGen` are wired (Phase 10). The §13.8 "what changes vs. today's dispatch" subsection is the precise delta — one `.pipeline_cls` attribute access in `AutoPipeline.from_config`, everything else additive.

Phase 11 is two click-option edits (`commands/serve.py:824-829`, `bench/benchmark/visual_gen.py:57-64`) using click's two-name option pattern. No new mechanisms.

A natural batching pattern: a round may land AC-N alone or batch AC-N + AC-N+1 when they share the same files (e.g. AC-2 sub-config-attr renames + AC-3 master-switch renames both edit `args.py` and `pipeline_loader.py` and can be one diff). The `CHANGES REQUIRED` rounds will naturally surface when batched scope is too aggressive.

### Relevant References

- `tensorrt_llm/_torch/visual_gen/config.py` — current location of `VisualGenArgs` and sub-configs; this file shrinks dramatically (most of its content moves to `tensorrt_llm/visual_gen/args.py`).
- `tensorrt_llm/_torch/visual_gen/pipeline_registry.py` — current registry; upgrade in place per §13.8.
- `tensorrt_llm/_torch/visual_gen/pipeline_loader.py` — main consumer; absorbs new strict-validation step and reads the env var; switches from `args.compilation.skip_warmup` (or top-level `args.skip_warmup`) to `args.compilation_config.skip_warmup`.
- `tensorrt_llm/_torch/visual_gen/auto_pipeline.py` (or wherever `AutoPipeline.from_config` lives) — one-line delta to read `PIPELINE_REGISTRY[class_name].pipeline_cls`.
- `tensorrt_llm/_torch/visual_gen/pipeline_wan.py`, `pipeline_flux.py`, `pipeline_ltx2.py` (and similar) — decorator metadata sites for Phase 9.
- `tensorrt_llm/_torch/visual_gen/mapping.py` — `DEFAULT_DIM_ORDER` already defined; consumer for unpacked `attn2d_size`.
- `tensorrt_llm/llmapi/llm_args.py` — reference for `Field(status=...)` wrapper, `StrictBaseModel`, and the canonical `_config`-suffix layout to mirror.
- `tensorrt_llm/commands/serve.py:824-829` — VisualGen CLI flag declaration.
- `tensorrt_llm/bench/benchmark/visual_gen.py:57-64` — bench CLI flag declaration.
- `tensorrt_llm/__init__.py` — top-level re-export list; verify the seven names in §13.1 match current state before publishing.
- `tests/unittest/_torch/visual_gen/test_visual_gen_args.py` — Pydantic round-trip; primary target for AC-2 / AC-3 / AC-4 negative-test assertions.
- `tests/unittest/_torch/visual_gen/test_fused_qkv.py` — references the deleted `fuse_qkv`; remove or rewrite per AC-4.
- `tests/unittest/_torch/visual_gen/test_flux_pipeline.py:60`, `test_cache_dit.py:263-266` — `skip_components` callers; migrate to `TLLM_VG_SKIP_COMP` env-var fixture per AC-6.2.
- `examples/visual_gen/`, `examples/configs/database/` — YAML configs likely to reference renamed fields; sweep before declaring the loop done.

## Dependencies and Sequence

### Milestones

The 11 ACs group naturally into 6 milestones; each milestone is one or more RLCR rounds.

1. **Foundation** — establish the new public layout.
   - Phase 1 / AC-1: Move `VisualGenArgs` and sub-configs to `tensorrt_llm/visual_gen/args.py`, adopt `Field(status="prototype")`.
   - Phase 2 / AC-2: Rename sub-config attributes to `_config` suffix.

2. **Cross-cutting field renames** — internal consistency with `LlmArgs` conventions.
   - Phase 3 / AC-3: Rename `enable_cuda_graph` / `enable_torch_compile` → `enable`; add `CompilationConfig.skip_warmup`.

3. **Dead-code and internal-state cleanup** — shrink the public surface.
   - Phase 4 / AC-4: Delete dead fields; move `dit_dim_order` to internal constant.
   - Phase 5 / AC-5: Move derived quant flags off the public schema.

4. **Debug-knob relocation** — restore semantic homes for the three knobs.
   - Phase 6 / AC-6: `skip_warmup` (compilation_config), `skip_components` (env var), `enable_layerwise_nvtx_marker` (top-level flat).

5. **Per-architecture knob plumbing** — the heart of the refactor.
   - Phase 7 / AC-7: Upgrade `pipeline_registry.py` to carry `_PipelineEntry` metadata; extend `@register_pipeline`.
   - Phase 8 / AC-8: Add `pipeline_config: dict[str, Any]` with strict load-time validation.
   - Phase 9 / AC-9: Fill per-family `@register_pipeline` kwargs; move LTX-2 paths into registry defaults; rename `ParallelConfig` survivors.

6. **Discovery and CLI** — make the new shape findable.
   - Phase 10 / AC-10: `VisualGen.supported_models()` + `VisualGen.pipeline_config(model)`.
   - Phase 11 / AC-11: `--visual_gen_args` primary, `--extra_visual_gen_options` alias.

### Dependency relations

- AC-2 depends on AC-1 (the renamed attributes need somewhere to live).
- AC-3 depends on AC-2 (the master-switch renames assume the parent sub-configs are already at their new attribute names).
- AC-6.1 depends on AC-3 (`compilation_config.skip_warmup` only exists after AC-3 adds the field).
- AC-8 depends on AC-7 (the registry entry shape must already carry `defaults` for strict-validation to have something to check against).
- AC-9 depends on AC-7 + AC-8 (the per-family metadata sits on the new `_PipelineEntry`; the field-move into `defaults` makes sense only once `pipeline_config` is a real dict on `VisualGenArgs`).
- AC-10 depends on AC-7 + AC-9 (the classmethods read `PIPELINE_REGISTRY` for `hf_ids` / `defaults`; both must be populated).
- AC-11 is independent — can land in any order relative to others, but landing it last lets the help text reference the final shape.
- AC-4 and AC-5 are independent of everything except AC-1 (they edit the moved class) and of each other.

A reasonable round-order that respects dependencies and natural batching: AC-1, AC-2, AC-3 (+ AC-6.1 if `skip_warmup` migration batches in cleanly), AC-4, AC-5, AC-6.2 + AC-6.3, AC-7, AC-8, AC-9 (split into 9.1 / 9.2 / 9.3 if one round is too large), AC-10, AC-11. RLCR's `CHANGES REQUIRED` verdicts will surface when a single round is too aggressive — drop scope and try again.
...
- Code comments default to *none*. The source design is the engineering record; only add a code comment when the WHY is non-obvious (a hidden constraint, a subtle invariant, a workaround). Don't reference the loop, the design doc, or this plan from code comments.

<!-- impl-design: coding voice (only for tracked files) -->
## Coding Voice for Tracked Output

Tracked output (commits, code, code comments) is the engineering
record. Loop bookkeeping is not.

- **Do not** reference acceptance-criterion IDs (`AC-1`, `AC-3`) or
  RLCR priority tags (`[P0]`–`[P9]`) in commit messages, code, or
  code comments. Those belong to the loop and live in `.humanize/`.
- Commit messages and any new comments read as engineer-to-engineer:
  state what changed and why, not which review item it satisfies.
- Codex's raw review output (`round-N-review-result.md`) keeps its
  `[P0-9]` markers untouched — that's RLCR's machine contract.

<!-- /impl-design: coding voice -->

<!-- impl-design: build & test invocation -->
## Build & Test Invocation (Per Round)

Build and test commands run through the `run_in_gpu_env` helper that
impl-design's Stage 2 binds at entry. The helper tees output to
`logs/round-<N>-<step>-<ts>.log` (relative to this loop dir under
`.humanize/rlcr/<ts>/`) and returns distinguishable exit codes.

Per-round protocol:

- After implementing this round's tasks, run the AC tests called for
  in the plan (or the natural subset triggered by your changes) via
  `run_in_gpu_env`. Codex does not re-run tests — it trusts your
  summary, and your summary must point to actual logs.
- In `round-N-summary.md`, list each invocation with its
  `logs/round-N-<step>-<ts>.log` path, the exit code, and a minimal
  excerpt of the relevant pass/fail output.
- Build is required only when non-`.py` files in the worktree changed
  since the last successful build. Python-only changes are live via
  the editable install (`pip install -e`) — skip the rebuild and note
  it in the summary so Codex can confirm the diff is `.py`-only.
- Build args: **Release** + Ninja + SM100 + `CMAKE_EXPORT_COMPILE_COMMANDS=ON`
  (impl-design override of the CLAUDE.md Debug default; long loops
  compile many times). Not `build_wheel.py` defaults.
