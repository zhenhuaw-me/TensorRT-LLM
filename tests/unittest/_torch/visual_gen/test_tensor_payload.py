...
:meth:`VisualGenOutput.save` and :meth:`VisualGenOutput.save_bytes`
...
class TestSingleSavePath:
    """A single path writes one logical output, matching the image and
    video encoder semantics — batched tensors collapse to the first
    slice, unbatched tensors are written as-is."""
    ...
    def test_batched_image_writes_first_item_only(self, fmt, tmp_path):
        output = _make_image_output(batch=3)
        target = tmp_path / "img"
        saved = output.save(target, format=fmt)
        loaded = (_safetensors_load if fmt == "safetensors" else _pt_load)(saved.read_bytes())
        # Per the encoder convention, single-path save corresponds to
        # the leading batch item only.
        assert loaded["image"].shape == (8, 8, 3)
        assert torch.equal(loaded["image"], output.image[0])

    def test_batched_video_writes_first_item_and_audio(self, fmt, tmp_path):
        output = _make_video_output(batch=2)
        target = tmp_path / "vid"
        saved = output.save(target, format=fmt)
        loaded = (_safetensors_load if fmt == "safetensors" else _pt_load)(saved.read_bytes())
        # Video first item is shape (T, H, W, C); audio first item is
        # (channels, T_audio).
        assert loaded["video"].shape == (2, 4, 4, 3)
        assert loaded["audio"].shape == (2, 32)
        if fmt == "pt":
            # ``pt`` preserves scalar metadata as native Python types.
...
@pytest.mark.parametrize("fmt", ["safetensors", "pt"])
class TestSaveBytes:
    """``save_bytes`` returns the same payload as :meth:`save` for the
    bytes-based transport."""

    def test_batch_index_slices_image(self, fmt):
        output = _make_image_output(batch=2)
        for i in range(2):
            data = output.save_bytes(fmt, batch_index=i)
    ...
    def test_batch_index_none_writes_unbatched_as_is(self, fmt):
        ...
        data = output.save_bytes(fmt, batch_index=None)
        loaded = (_safetensors_load if fmt == "safetensors" else _pt_load)(data)
...
class TestSaveMetadataOverrides:
    ...
    def test_save_overrides_unset_metadata(self, fmt, tmp_path):
        ...
        if fmt == "pt":
            ...
            # safetensors metadata is exposed via the file-level
            # ``metadata`` header; using ``save_bytes`` exercises the
            # same code path and round-trips the keys as strings.
            data_bytes = output.save_bytes(
    ...
    def test_save_bytes_overrides(self, fmt):
        """``save_bytes`` honors the same overrides for the
        ``b64_json`` transport."""
        video = torch.zeros(1, 2, 4, 4, 3, dtype=torch.uint8)
        output = VisualGenOutput(request_id=1, video=video)
        data = output.save_bytes(fmt, batch_index=0, frame_rate=30.0, audio_sample_rate=44100)
        if fmt == "pt":
