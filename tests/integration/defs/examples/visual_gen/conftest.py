...
def _install_ffmpeg_via_apt():
    ...
    # either the index or the .deb download, and repair any dpkg left interrupted
    # by a killed prior attempt so retries stay useful.
    install_cmd = (
        "dpkg --configure -a || true; "
        'opts="-o Acquire::http::Timeout=30 -o Acquire::https::Timeout=30"; '
        "apt-get $opts update || true; "
        "DEBIAN_FRONTEND=noninteractive apt-get $opts install -y ffmpeg"
    )
