...
| 5 | **Shared configs** | Both tiers consume the same YAML config files. `VisualGenArgs.from_yaml()` in example scripts; `trtllm-serve --extra_visual_gen_options` in serve. |
...
| 8 | **Examples use the API, not CLI** | Per-model examples focus on model-specific request construction and output processing. The only CLI flag is `--extra_visual_gen_options` (optional, shared with `trtllm-serve`). No per-model CLI abstraction. |
...
| `argparse` flags for parallelism, quant, TeaCache | Single `--extra_visual_gen_options` flag pointing to shared YAML config (optional) | Same config for offline examples and `trtllm-serve`, no per-model CLI layer |
...
(`--extra_visual_gen_options`) and `trtllm-serve`
...
flag is `--extra_visual_gen_options`, which optionally points to a YAML config
...
Usage:
    python wan_t2v.py
    python wan_t2v.py --extra_visual_gen_options ../configs/wan_t2v_8gpu.yaml
"""
import argparse
...
parser.add_argument("--extra_visual_gen_options", type=str, default=None,
...
`--extra_visual_gen_options` to a performance config to see tuned behavior:
...
| Only CLI flag is `--extra_visual_gen_options` (optional) | Shared with `trtllm-serve --extra_visual_gen_options`; no per-model CLI abstraction |
...
| Omission | Reason |
|----------|--------|
| Per-feature example pages (quantization, TeaCache, parallelism) | Not enough features yet to justify separate pages. These are YAML config fields passed via `--extra_visual_gen_options`. |
...
Per-model examples have only one optional CLI flag (`--extra_visual_gen_options`),
...
def test_wan_t2v_perf_config(llm_venv):
    ...
    venv_check_call(llm_venv, ["python", str(script),
                               "--extra_visual_gen_options", str(config)])
    assert Path("wan_t2v_output.mp4").exists()
```
...
|---|----------|---------|---------|
| 1 | **Should the model path in per-model scripts be editable inline or also a CLI flag?** | (a) inline only (user edits the script), (b) `--model_path` flag alongside `--extra_visual_gen_options` | (a) — keeps scripts minimal; CI can use the default HF ID or symlink. If CI needs a local path, (b) adds one more flag without bloating the script. |
...
| 6 | **When to add per-feature examples?** | (a) now, (b) when we have 3+ features that need dedicated pages | (b) — premature now. Quant, TeaCache, parallelism are YAML config fields passed via `--extra_visual_gen_options`. |
