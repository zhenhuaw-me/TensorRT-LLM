...
#### The "X prompts and Y images" as inputs where `X != Y`

We are discussing the `VisualGen.generate(inputs, params)` API — specifically, the design of `(VisualGenInputs, VisualGenParams)`. The main design choice is **whether to put the image in the Inputs or in the Params.**

The generation with images can be viewed as "X prompts and Y images," giving these use cases:

1. `X == Y == 1`: the simplest use case. Either placement works. Since the image is optional for T2V, it's more reasonable to put `image` in `VisualGenParams` — for consistency with the LLM API convention.
2. `X == Y, X != 1, Y != 1`: the typical "multi-batch" use case. Either placement works. Again, since the image is optional for T2V, it's more reasonable to put `image` in `VisualGenParams` — for the LLM API convention.
3. `X != 1, Y == 1`: a single image with multiple prompts (context). To follow LLM's convention, we should put `image` in `VisualGenParams`. If we put it in `Inputs`, the `Inputs(prompts, images)` class and the `generate()` API become tricky to define — how do we match images to the per-request params (e.g., H/W/steps)?
4. `X == 1, Y != 1`: we don't see such cases yet.
...
# Batch with per-request params — DEFERRED (see §5.1.2 "Batching Support Scope").
# The signature type-checks it, but the current milestone raises
# NotImplementedError; only "N prompts + 1 params" is supported today.
...
#### 5.1.2 Batching Support Scope (Current Milestone)

The `generate()` signature above is typed as `Optional[Union[VisualGenParams, List[VisualGenParams]]]` for forward compatibility with the LLM API pattern, but **only "N prompts + 1 params" (a single shared `VisualGenParams` across the batch) is supported in this milestone**. Passing `List[VisualGenParams]` with per-request parameters raises `NotImplementedError`.

**Why the restriction:**

- The internal scheduler/executor and model pipelines currently assume uniform generation settings across a batch (shared resolution, steps, guidance, etc.) because the underlying kernels batch tensors on shape-compatible inputs.
- Supporting `List[VisualGenParams]` requires:
  1. API-level validation that `len(params) == len(prompts)`.
  2. Executor changes to split a heterogeneous batch into homogeneous sub-batches (by resolution / step count / guidance) before dispatch.
  3. Pipeline support for merging per-request outputs back into a single result list, preserving prompt order.
- These changes cut across `DiffusionExecutor`, the scheduler, and each `BasePipeline.infer()`; they are deferred to a follow-up iteration.

**What users should do today:**

- For shared settings: pass a list of prompts + a single `VisualGenParams`. This is the supported batch path.
- For truly heterogeneous requests: issue them as separate `generate_async()` calls; the scheduler still overlaps them across worker steps.

**API evolution path**: Once inference-engine support lands, the `List[VisualGenParams]` form becomes a pure additive change — no user-facing signature change beyond lifting the `NotImplementedError`.
...
#### 6.1.1 Batch Output Shape: `List[VisualGenOutput]` vs Batched Tensor

When `generate()` receives N prompts, should it return **N separate outputs** (each carrying an unbatched tensor) or **one batched output** (a single tensor whose leading dim is N)? This section surveys the peer frameworks and recommends from a user perspective.

#### How Peer Frameworks Return Batches

| Framework        | Image batch of N                                            | Video/frames batch of N                                       | Shape model                |
| :--------------- | :---------------------------------------------------------- | :------------------------------------------------------------ | :------------------------- |
| **Diffusers**    | `FluxPipelineOutput.images: list[PIL.Image] \| np.ndarray`  | `WanPipelineOutput.frames: torch.Tensor` (leading dim = N)    | Hybrid — list for PIL, batched tensor for `pt`/`np` |
| **vLLM-Omni**    | `OmniDiffusionResponse` yields N base64 PNGs (OpenAI shape) | Same — per-item encoded bytes                                 | N independent items (list) |
| **SGLang Diffusion** | File path or frames per request (one per prompt)        | File path / frames per request                                | N independent items (list) |
| **LLM API (TRT-LLM)** | `List[RequestOutput]`, one per prompt                 | —                                                             | N independent items (list) |

Key observation: every peer framework **except** diffusers-in-tensor-mode returns N independent items. Diffusers is the outlier, and even there `.images` on an image pipeline is a Python `list[PIL.Image]` by default — only when users explicitly ask for `output_type="pt"` / `"np"` do they get a single batched tensor.

#### Option A — `List[VisualGenOutput]` (one unbatched tensor per item)

```python
results: List[VisualGenOutput] = engine.generate(["A cat", "A dog", "A fox"], params=params)
# each result.output.video has shape (T, H, W, C) — no leading batch dim
results[0].output.save("cat.mp4")
results[1].output.save("dog.mp4")
results[2].output.save("fox.mp4")
```

**Pros:**

- **1-to-1 mapping**: each `VisualGenOutput` carries its own `prompt`, `seed_used`, `metrics`, `error`, and media — no cross-item coupling. This matches the LLM API's `List[RequestOutput]` contract, so a user who knows `LLM` predicts this shape for `VisualGen`.
- **Natural save/serve loop**: `for r in results: r.output.save(...)` is the common case; no manual slicing.
- **Heterogeneous shapes allowed**: even under uniform params today, future per-item params (see §5.1.2) or variable-length video wouldn't break the contract.
- **Per-item error model**: Option B in §5.1.1 ("return all with per-item errors") requires one `VisualGenOutput` per prompt anyway; a single batched tensor can't represent a partial failure.
- **Serving layer is already per-request**: OpenAI/DALL-E-style endpoints emit one response object per prompt; a single batched tensor would be decomposed immediately.

**Cons:**

- Slight Python-list overhead vs a contiguous tensor (negligible for visual generation, which runs at single-digit QPS and seconds-to-minutes latency per request).
- Users who want a single stacked tensor must `torch.stack([r.output.video for r in results])` themselves — one line.

#### Option B — Single `VisualGenOutput` with a batched tensor (leading dim = N)

```python
result: VisualGenOutput = engine.generate(["A cat", "A dog", "A fox"], params=params)
# result.output.video has shape (N, T, H, W, C)
for i in range(result.output.video.shape[0]):
    MediaOutput(video=result.output.video[i]).save(f"{i}.mp4")
```

**Pros:**

- One contiguous tensor in GPU/host memory — slightly faster for downstream bulk ops (`.mean()`, quality metrics across the batch).
- Matches diffusers' `WanPipelineOutput.frames` shape exactly for users porting code.

**Cons:**

- Breaks the 1-to-1 metadata mapping: where does `seed_used[i]` live? A parallel list? That's strictly worse than `List[VisualGenOutput]`.
- Assumes uniform shape across all items in the batch. Fine today (shared params, §5.1.2), but brittle when per-request params eventually land.
- User must slice to save individual items — the most common operation is now more verbose, not less.
- Partial-failure semantics are awkward: a tensor row for a failed item has no meaningful content; the error surface forks from the data.
- Diverges from the LLM API (`List[RequestOutput]`), raising the learning cost for users who use both.

#### Recommendation — Option A (`List[VisualGenOutput]`)

Return `List[VisualGenOutput]` from `generate()` when inputs are a list. Each `MediaOutput` inside carries an **unbatched** tensor (`(T, H, W, C)` for video, `(H, W, C)` for image). Rationale:

- **User intuition**: N prompts → N results. The 1-to-1 mapping matches how every peer framework (except a narrow diffusers mode) presents batches, and matches the LLM API convention already established in this codebase.
- **Metadata per item**: `seed_used`, `prompt`, `metrics`, `error` belong to a request, not a tensor row. The wrapper gives them a natural home.
- **Streaming-compatible**: future `stream()` yields incremental `VisualGenOutput` per request — the shape is already correct.
- **Performance is a non-issue**: at visual-gen throughput (single-digit QPS, multi-second latency), the cost of not having a contiguous batch is unmeasurable next to denoising. Users who genuinely need a stacked tensor can `torch.stack(...)` in one line.

**Implementation note**: Internally the model pipeline may well produce a batched tensor from a single forward pass — but the executor splits along the leading dim before constructing `List[VisualGenOutput]`. The internal representation and the user-facing shape are independent choices; we optimize the former for the GPU and the latter for the user.
...
#### Option C: New `tensorrt_llm/visual_gen/` as the public API re-export layer

Similar to Option A, but with a different dir name.
...
- It's an implementation change with no impact on the user experience of `from tensorrt_llm import VisualGen`, so it's safe to move.
- The `visual_gen` keyword is aligned with the other VisualGen components.
...
- With the decision to use plain `str` as the prompt type and place conditioning inputs (negative_prompt, image, mask) on `VisualGenParams` (see the "X prompts and Y images" analysis in §4.1), none of these structured input types are needed in the public API.
...
# Batch with shared params (one image reused across prompts)
results = engine.generate(
    ["A cat on a windowsill", "A dog in a park"],
    params=VisualGenParams(image="summer.jpg", height=480, width=832, seed=42),
)

# NOTE: batch with per-request params (below) is NOT supported in the
# current milestone — see §5.1.2 "Batching Support Scope". Supporting
# `List[VisualGenParams]` requires additional API + inference engine work
# and is deferred to a later iteration.
#
# results = engine.generate(
#     ["A cat", "A dog"],
#     params=[
#         VisualGenParams(image="baseline1.jpg", height=480, width=832),
#         VisualGenParams(image="baseline2.jpg", height=720, width=1280),
#     ],
# )
...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------- |
| **Init**          | Move `VisualGenArgs` + sub-configs from `_torch/` to `tensorrt_llm/visual_gen` (see §3.1, §9)                                                                                                      | High     | No (re-export preserves user imports) |
...
| **Request**       | Support single + batch via Union types (match LLM pattern). Current milestone: "N prompts + 1 params" only; `List[VisualGenParams]` raises `NotImplementedError` pending executor/pipeline work (see §5.1.2) | Medium   | Yes (signature)                       |
| **Output**        | Batch returns `List[VisualGenOutput]` with unbatched per-item tensors, matching LLM API and ecosystem conventions (see §6.1.1)                                                                     | High     | Contract                              |
