...
def _warn_if_set_with_no_semantic(
    ...
    if model_value is not None and loaded_model_id is not None and model_value != loaded_model_id:
        logger.warning(
            ...
            "%r; trtllm-serve is single-model per process and the field "
...
def _parse_image_edit_request(request: ImageEditRequest, generator: VisualGen) -> VisualGenParams:
    """Translate an :class:`ImageEditRequest` into :class:`VisualGenParams`.

    The ``/v1/images/edits`` route returns HTTP 501 today because no
    in-tree pipeline implements editing. This helper exists so the
    route can construct a request payload for the eventual edit
    pipeline without depending on the redesigned image-generation
    conversion above.
    """
    params = generator.default_params
    if request.negative_prompt is not None:
